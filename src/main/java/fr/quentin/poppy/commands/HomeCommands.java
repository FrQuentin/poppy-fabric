...
public class HomeCommands {
    ...
    private static int setHome(CommandContext<ServerCommandSource> context, String homeName) {
        try {
            ...
            Poppy.LOGGER.error("An error occurred while executing /sethome command for player {}", Objects.requireNonNull(context.getSource().getPlayer()).getName(), e);
            context.getSource().sendError(Text.translatable("commands.poppy.generic_error"));
        }
        return 1;
    }

    private static int deleteHome(CommandContext<ServerCommandSource> context, String homeName) {
        try {
            var source = context.getSource();
            var player = source.getPlayerOrThrow();
            ...
            Poppy.LOGGER.error("An error occurred while executing /delhome command for player {}", Objects.requireNonNull(context.getSource().getPlayer()).getName(), e);
            context.getSource().sendError(Text.translatable("commands.poppy.generic_error"));
        }
        return 1;
    }
    ...
    private static int listHomes(CommandContext<ServerCommandSource> context, int page) {
        try {
            var source = context.getSource();
            var player = source.getPlayerOrThrow();
            var playerId = player.getUuid();
            ...
            Poppy.LOGGER.error("An error occurred while executing /homes command for player {}", Objects.requireNonNull(context.getSource().getPlayer()).getName(), e);
            context.getSource().sendError(Text.translatable("commands.poppy.generic_error"));
        }
        return 1;
    }
